...
import { Heart, Sparkles, MapPin, Star, Clock, ShoppingBag, Leaf, Flame, Info, ChevronDown, ChevronUp, Utensils } from 'lucide-react';
            ...
            return (
              ...
              >
                ...
                <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
                  <div>
                    {/* Header line: Title & Quick Details Toggle */}
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <h3 className="font-black italic text-gray-900 text-lg sm:text-xl leading-tight group-hover:text-orange-600 transition-colors">
                        {dish.name}
                      </h3>
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          setExpandedDishId(prev => prev === dish.id ? null : dish.id);
                        }}
                        className="p-1.5 bg-gray-50 hover:bg-orange-50 text-gray-400 hover:text-orange-600 rounded-full transition-colors shrink-0"
                        title="Dettagli e Ingredienti"
                      >
                    ...
                    {/* Short Description */}
                    <p className="text-gray-500 text-xs sm:text-sm leading-relaxed mb-3 line-clamp-2 italic font-medium">
                      "{dish.description}"
                    </p>

                    {/* Collapsible Info / Ingredients Box */}
                    {expandedDishId === dish.id && (
                      <div className="mb-4 p-3 bg-orange-50/70 border border-orange-100 rounded-2xl space-y-2 text-xs animate-fade-in" onClick={(e) => e.stopPropagation()}>
                        {dish.ingredients && dish.ingredients.length > 0 && (
                          <div>
                            <span className="font-black text-orange-900 uppercase tracking-wider text-[9px] flex items-center gap-1 mb-0.5">
                              <Leaf className="w-3 h-3 text-green-600" /> Ingredienti:
                            </span>
                            <p className="text-gray-700 font-medium">{dish.ingredients.join(', ')}</p>
                          </div>
                        )}
                        {dish.preparation && (
                          <div className="pt-1.5 border-t border-orange-200/50">
                            <span className="font-black text-orange-900 uppercase tracking-wider text-[9px] flex items-center gap-1 mb-0.5">
                              <Utensils className="w-3 h-3 text-orange-600" /> Note dello Chef:
                            </span>
                            <p className="text-gray-700 italic">{dish.preparation}</p>
                          </div>
                        )}
                        <div className="pt-1.5 border-t border-orange-200/50 flex items-center justify-between text-[10px] text-gray-600 font-bold">
                          <span>Porzione: ~350g (Singola)</span>
                          <span className="text-orange-700 font-black">Allergeni Segnalati</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-5">
                      <span className="px-2.5 py-1 bg-orange-50 text-orange-800 rounded-xl text-[9px] font-black uppercase tracking-wider flex items-center gap-1">
                        <Clock className="w-3 h-3 text-orange-600" /> 25-35 min
                      </span>
                      ...
                      {dish.tags.map(tag => (
                        <span key={tag} className="px-2.5 py-1 bg-gray-50 border border-gray-100 rounded-xl text-[9px] font-black uppercase tracking-widest text-gray-500">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="space-y-3.5">
                    {associatedChef && (
                      <div 
                        ...
                        className="flex items-center justify-between p-3 bg-gray-50/80 hover:bg-orange-50/60 rounded-2xl border border-gray-100 transition-colors group/chef cursor-pointer"
                      >
                        <div className="flex items-center gap-2.5">
                          <img 
                            src={dish.chefAvatar} 
                            alt={dish.chefName}
                            className="w-8 h-8 rounded-xl object-cover border border-white shadow-sm shrink-0"
                          />
                          <div className="min-w-0">
                            <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">Chef Locale</p>
                            <span className="text-xs font-bold text-gray-800 group-hover/chef:text-orange-600 transition-colors truncate block">{dish.chefName}</span>
                          </div>
                        </div>

                        {/* Chef stats */}
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="flex items-center gap-0.5 bg-amber-400/15 text-amber-800 px-2 py-0.5 rounded-lg text-[9px] font-black">
                    ...
                    {/* Book Now Button */}
                    <button className="w-full py-3.5 rounded-2xl bg-orange-600 hover:bg-orange-700 text-white font-black uppercase tracking-widest text-[10px] sm:text-xs flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 border-b-4 border-orange-850 hover:border-orange-950">
