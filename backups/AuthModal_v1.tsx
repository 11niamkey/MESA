...
export const AuthModal: React.FC<AuthModalProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Accesso Segreto Amministratore (solo per il proprietario)
    if (email.toLowerCase() === 'admin@mesa.com' || email.toLowerCase() === '11niamkey@gmail.com') {
      if (password === 'mesa2026') {
        const adminUser = registeredUsers.find(u => u.role === 'admin' && u.email.toLowerCase() === 'admin@mesa.com');
        if (adminUser) {
        ...
        // Se provano ad accedere con l'email ma sbagliano pass, li blocchiamo senza rivelare nulla o diamo errore
        setErrorMsg('Credenziali amministratore non valide.');
        return;
      }
    }

    if (role === 'admin') {
      setErrorMsg('Accesso amministratore tramite pannello disabilitato. Usa le credenziali segrete.');
      return;
    }
