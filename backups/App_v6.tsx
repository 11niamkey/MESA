...
function App() {
  ...
  const [chefApplications, setChefApplications] = useState<ChefApplication[]>(() => {
    const saved = localStorage.getItem('mesa_chef_applications');
  ...
  const handleChefSubmitApplication = (newApp: ChefApplication) => {
    setChefApplications(prev => [newApp, ...prev.filter(a => a.id !== newApp.id)]);
  ...
  const handleApproveApplication = (appId: string) => {
    ...
    if (app) {
      ...
      setRegisteredUsers(prev => {
        const existingUser = prev.find(u => u.email.toLowerCase() === app.email.toLowerCase());
        if (existingUser) {
          return prev.map(u => u.id === existingUser.id ? { ...u, role: 'chef' as const } : u);
        } else {
      ...
      setChefApplications(prev => prev.map(a => a.id === appId ? { ...a, status: 'approved', reviewedAt: new Date().toLocaleString('it-IT') } : a));
      showNotification(`Profilo di ${app.brandName} valutato ed attivato nel catalogo MESA! 🎉`);
    }
  };

  const handleRejectApplication = (appId: string, notes?: string) => {
    setChefApplications(prev => prev.map(a => a.id === appId ? { ...a, status: 'rejected', adminNotes: notes || 'Documenti incompleti', reviewedAt: new Date().toLocaleString('it-IT') } : a));
