...
function App() {
  ...
  const [registeredUsers, setRegisteredUsers] = useState<AppUser[]>(() => {
    const saved = localStorage.getItem('mesa_registered_users');
    const existing = saved ? JSON.parse(saved) : DEFAULT_USERS;
  ...
  useEffect(() => {
    localStorage.setItem('mesa_registered_users', JSON.stringify(registeredUsers));
  }, [registeredUsers]);
  ...
  const handleVerifyAddress = (userId: string, addressId: string, isVerified: boolean) => {
    setRegisteredUsers(prev => prev.map(u => {
      if (u.id === userId) {
        return {
          ...u,
  ...
  const navigateTo = (view: ViewState) => {
    if ([ViewState.ORDERS, ViewState.PROFILE, ViewState.ADMIN_DASHBOARD].includes(view) && !currentUser) {
    ...
    if (view === ViewState.ADMIN_DASHBOARD && currentUser?.role !== 'admin') {
      showNotification("Errore: Accesso negato all'area amministratore.");
      return;
    }
  ...
  const handleQuickAdminAccess = () => {
    ...
    showNotification("Accesso effettuato come Amministratore (Admin Demo)! 🔑");
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handlePaymentSuccess = (notificationChannel?: 'app' | 'whatsapp') => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col pb-20 md:pb-0 transition-colors duration-500">
      <Header 
        cartCount={cartCount} 
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 flex-grow w-full mb-8">
        ...
        {[ViewState.ABOUT, ViewState.SAFETY, ViewState.BECOME_CHEF, ViewState.HELP, ViewState.TERMS, ViewState.PRIVACY].includes(currentView) && (
          <InfoPage view={currentView} onBack={() => navigateTo(ViewState.HOME)} />
        )}
      </main>
      ...
      {/* Glovo-style Sticky Floating Cart Bar */}
      {cartCount > 0 && !isCartOpen && (
        <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-lg bg-orange-600 hover:bg-orange-700 text-white p-4 rounded-2xl sm:rounded-3xl shadow-2xl shadow-orange-950/30 border border-orange-500 flex items-center justify-between cursor-pointer transition-all transform hover:scale-[1.02] active:scale-95 animate-fade-in-up" onClick={() => setIsCartOpen(true)}>
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="bg-white text-orange-600 text-xs font-black px-2 py-0.5 rounded-full">{cartCount}</span>
                <span className="font-extrabold text-sm">{cartCount === 1 ? 'Piatto nel carrello' : 'Piatti nel carrello'}</span>
              </div>
              <span className="text-[11px] text-orange-100 font-bold">Consegna diretta o Ritiro</span>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <span className="text-lg font-black font-mono">€ {cart.reduce((sum, item) => sum + (item.price * item.quantity), 0).toFixed(2)}</span>
            <span className="bg-white/20 hover:bg-white/30 text-white p-2 rounded-xl text-xs font-black uppercase tracking-wider">Vedi →</span>
          </div>
        </div>
      )}
      ...
      <ChefOnboarding isOpen={isOnboardingOpen} onClose={() => setIsOnboardingOpen(false)} onChefCreate={handleAddChef} currentUser={currentUser} />
      <NotificationToast message={toastMessage} isVisible={isToastVisible} onClose={() => setIsToastVisible(false)} />
