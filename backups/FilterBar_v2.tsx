    ...
    <div id="main-filter-bar" className="bg-white border-b border-orange-100 py-6 sticky top-20 z-40 shadow-sm animate-fade-in">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mt-6 pt-2">
          ...
          {/* Tradizioni dal Mondo con Bandiere, Nomi e Miniature di Piatti Tipici */}
          <div>
            <div className="flex items-center justify-between mb-3 px-1">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-1.5 animate-pulse">
                🌎 Esplora Cucina per Origine e Sapori
              </span>
              {filters.countries.length > 0 && (
                <button 
                  onClick={() => onFilterChange({ ...filters, countries: [] })}
                  className="text-[9px] font-black text-orange-600 uppercase hover:underline"
                >
                  Svuota Origini
                </button>
            ...
            <div className="flex overflow-x-auto space-x-4 pb-4 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
              {countryCuisines.map(item => {
                const isActive = filters.countries.includes(item.flag);
                const countryName = nationalityNames[item.flag]?.[0] || item.name;
                return (
                  <button
                    key={item.flag}
                    onClick={() => toggleCountry(item.flag)}
                    className={`relative flex-none w-44 h-24 rounded-[1.5rem] overflow-hidden border-2 transition-all group cursor-pointer ${
                      isActive 
                        ? 'border-orange-500 shadow-xl scale-105 ring-4 ring-orange-500/15' 
                        : 'border-transparent hover:border-orange-200 hover:shadow-md'
                  ...
                  >
                    ...
                    />
                    <div className={`absolute inset-0 bg-gradient-to-t transition-opacity duration-300 ${
                      isActive ? 'from-orange-600/95 to-black/35 opacity-95' : 'from-black/80 via-black/45 to-transparent'
                    }`}></div>
