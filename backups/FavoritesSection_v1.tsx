...
export const FavoritesSection: React.FC<FavoritesSectionProps> = ({ 
  ...
  if (favorites.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center animate-fade-in font-sans">
        <div className="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mb-6">
          <Heart className="w-10 h-10 text-gray-300" />
        </div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2 font-sans">Ancora nulla nei preferiti</h2>
  ...
  return (
    <div className="animate-fade-in px-2 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {favoriteDishes.map((dish) => (
          <div key={dish.id} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl transition-all duration-300">
            ...
            <div className="p-7 flex flex-col h-full col">
              ...
              <button 
                onClick={() => onAddDish(dish)}
                className="w-full py-4 bg-black text-white font-black uppercase tracking-widest rounded-2xl flex items-center justify-center space-x-3 group-hover:bg-orange-600 transition-all shadow-lg active:scale-95"
              >
