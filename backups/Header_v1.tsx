...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="sticky top-0 z-50 bg-black text-white border-b border-orange-900/30 shadow-lg transition-colors duration-500">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <div className="w-16 h-16 rounded-xl overflow-hidden bg-black shadow-lg shadow-orange-900/20 group-hover:scale-105 transition-all outline outline-1 outline-white/10 ml-[-4px] my-1">
            ...
            {activeAddress && (
              <button 
                ...
                className="flex flex-col text-left ml-1 sm:ml-3 py-1 px-2.5 sm:px-3 bg-neutral-900 hover:bg-neutral-800 border border-white/5 hover:border-orange-500/40 rounded-xl cursor-pointer transition-all shrink text-ellipsis whitespace-nowrap overflow-hidden max-w-[130px] sm:max-w-[200px]"
                title="Cambia indirizzo di consegna"
              >
                <div className="flex items-center gap-1 text-orange-500">
                  <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                  <span className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest text-green-400">Copertura</span>
                </div>
                <span className="text-[10px] sm:text-[11px] font-black text-gray-200 font-sans truncate">
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            <div className="flex items-center space-x-1 md:space-x-2">
              <button 
                ...
                className="flex items-center gap-1 md:gap-2 px-2.5 py-1.5 md:px-4 md:py-2 bg-orange-600 text-black text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-white transition-all shadow-lg active:scale-95"
                title="Condividi"
              >
              ...
              {onQuickAdminAccess && !hideAdmin && (
                <button
                  ...
                  className={`flex px-2 py-1.5 md:px-3 md:py-1.5 border leading-none font-black uppercase tracking-widest rounded-xl transition-all shadow-md hover:scale-105 active:scale-95 items-center gap-1 sm:gap-1.5 ${
                    currentUser?.role === 'admin'
                      ? 'bg-orange-500 hover:bg-orange-600 border-orange-600 text-black text-[10px]' 
                      : 'bg-neutral-900/80 hover:bg-neutral-800 border-orange-500/30 text-white text-[10px] sm:text-[10px] animate-pulse'
                  }`}
                ...
                >
                  ...
                  <span className="hidden sm:inline">{currentUser?.role === 'admin' ? 'Area Admin' : 'Admin Login'}</span>
